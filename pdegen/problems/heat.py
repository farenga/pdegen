...
from .utils import create_dataset_directory_tree, midpoints
...
class Heat2D(Problem):
    ...
    DuDt - Laplacian(u(x))=f(x), x in Omega
    ...
    def set_domain(self,config):
        if config.domain == 'unitsquare':
            self.n = config.n
            self.mesh = RectangleMesh(Point(-1, -1), Point(1, 1), self.n, self.n)
            self.V = FunctionSpace(self.mesh, 'P', 1)
            self.bc = DirichletBC(self.V, Constant(0), 'on_boundary')
        else:
            raise SyntaxError('only unitsquare is available as domain type')


    def solve(self):
        for i,mu in enumerate(self.param_instances):
            print('Solving for parameters instance #',i,' :', mu)

            if self.save_vtk:
                vtkfile = File(os.path.join(self.directory,'vtk','solution_'+str(i),'solution.pvd'))

            # Define initial value
            u_0 = Expression('exp(-(pow(x[0], 2) + pow(x[1], 2))/a)', degree=2, a=.2)
            ...
            lhs = u*v*dx + mu*self.dt*dot(grad(u), grad(v))*dx 
